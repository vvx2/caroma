...
if ($login == 0) {
    ...
    $opt = 's.admin_id = ? && gl.state_id = ? && s.status = ? order by check_fee ASC';
    $arr = array($admin_id, $state, 1);
    $shipping = $db->advwhere($col, $table, $opt, $arr);
    // var_dump($shipping);
    if (count($shipping) <= 0) {
        $json_arr = array('Status' => false, 'Msg' => 'Shipping Fee is not Available! Please Contact Admin');
    } else {
        $shipping = $shipping[0];
